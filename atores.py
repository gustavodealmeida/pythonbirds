...
class Ator():
    ...
    def esta_dentro_intervalo(self, pos1, pos2, intervalo):
    ...
    def colidir(self, outro_ator, intervalo=1):
        ...
        colidiu_na_horizontal = self.esta_dentro_intervalo(self.x, outro_ator.x, intervalo)
        colidiu_na_vertical = self.esta_dentro_intervalo(self.y, outro_ator.y, intervalo)
...
class Passaro(Ator):
    ...
    def calcular_posicao_x(self, delta_t):
        x = self._x_inicial + self.velocidade_escalar * delta_t* math.cos(self._angulo_de_lancamento)
        return x

    def calcular_posicao_y(self, delta_t):
        y = self._y_inicial + self.velocidade_escalar * delta_t* math.sin(self._angulo_de_lancamento)
    ...
    def calcular_posicao(self, tempo):
        ...
        if self.foi_lancado() and self.status == ATIVO:
            ...
            self.x = self.calcular_posicao_x(delta_t)
            self.y = self.calcular_posicao_y(delta_t)
        return self.x, self.y
